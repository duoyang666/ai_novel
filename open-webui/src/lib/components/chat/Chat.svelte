...
<script lang="ts">
	...
	import { search_web, search_db } from '$lib/apis/net';
	...
	const initNewChat = async () => {
		...
		let default_models = $models.find((m) => m.id == selectedModels[0]);
		console.log(default_models);
		$NetSettings['model'] = default_models; //只有添加$ 是实时的
		NetApi.is_connect();
	};
		...
		await Promise.all(
			selectedModelIds.map(async (modelId) => {
				...
				if (model) {
					...
					if (knowSearchEnabled) {
						//本地知识库搜索
						//let results = await search_db(prompt, model.id).catch((error) => {
						let results = await search_db(prompt, '').catch((error) => {
							console.log(error);
							toast.error(error);
							return null;
						});
							...
							responseMessage.citations = [
								{
									...
									source: { name: prompt, title: 'title', type: 'name' }
								}
							];
							messagesCT = { role: 'system', content: results.ct };
							messages = messages;
						}
					} else messagesCT = undefined;
	...
	const sendPromptOllama = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		if (messagesCT) {
			messages.splice(0, 0, messagesCT);
			messagesCT = undefined;
			scrollToBottom();
		}
		const messagesBody = [
		...
		files = [
			...files,
			...(lastUserMessage?.files?.filter(
				(item) => ['doc', 'file', 'collection'].includes(item.type) //'web_search_results'
			) ?? []),
			...(responseMessage?.files?.filter(
				(item) => ['doc', 'file', 'collection'].includes(item.type) //'web_search_results'
			) ?? [])
		].filter(
			// Remove duplicates
							...
							if (data.done == false) {
								...
								if ($settings.notificationEnabled && !document.hasFocus()) {
									const notification = new Notification(`${model.id}`, {
										...
										icon: `${WEBUI_OBJ.WEBUI2_BASE_URL}/static/favicon.png`
									});
								}
	...
	const sendPromptOpenAI = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		try {
			...
			const [res, controller] = await generateOpenAIChatCompletion(
				...
				`${WEBUI_OBJ.OPENAI2_API_BASE_URL}`
			);
	...
	const getWebSearchResults2 = async (model: string, parentId: string, responseId: string) => {
	...
	const getWebSearchResults = async (model: string, parentId: string, responseId: string) => {
		...
		responseMessage.statusHistory.push({
			...
			action: 'web_search',
			description: `搜索 "${prompt}"`
		});
		messages = messages;

		let results = await search_web(prompt, '').catch((error) => {
			//let results = await search_web(prompt, model).catch((error) => {
			console.log(error);
			toast.error(error);
			return null;
		});
		...
		if (results) {
			responseMessage.statusHistory.push({
				...
				action: 'web_search',
				description: $i18n.t('Searched {{count}} sites', { count: results.urls.length }),
				query: prompt,
				urls: results.urls,
