<script lang="ts">
	import { NetSettings, net_service, NetApi } from '$lib/constants';
	...
	//拉取自定义模型
	const pullModelHandler2 = async () => {
		const value1 = searchValue.trim().replace(/^ollama\s+(run|pull)\s+/, '');
		let obj = JSON.parse(JSON.stringify(items[0]));
		if (!obj) obj = {};
		obj.value = value1;
		obj.label = obj.value;
		obj.model.id = obj.value;
		obj.model.name = obj.value;
		obj.model.openai.id = obj.value;
		obj.model.is_net = true;
		//selectedModel = obj;
		value = value1;

		let net_items = [];
		let index = $models.find((item) => item.id === value);
		if (!index) {
			net_items.push(obj);
		}

		$NetSettings['net_service'] = true; //只有添加$ 是实时的
		$NetSettings['model'] = obj.model;
		$NetSettings['value'] = value;
		$NetSettings['name'] = 'pullModelHandler2';
		console.log(NetSettings);
		NetSettings.set({ ...$NetSettings });
		net_service.set(true);
		NetApi.is_connect();

		let new_items = [...items.map((c) => c.model), ...net_items.map((c) => c.model)];
		models.set([...new_items]);
		show = false;
	};

	onMount(async () => {
		ollamaVersion = await getOllamaVersion(localStorage.token).catch((error) => false);
		$NetSettings['model'] = selectedModel['model'];
	});
