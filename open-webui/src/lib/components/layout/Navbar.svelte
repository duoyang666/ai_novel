<script lang="ts">
	...
	import { NetSettings, net_service } from '$lib/constants';

	const i18n = getContext('i18n');

	export let initNewChat: Function; //从父组件传递的
	...
	<div class=" flex max-w-full w-full mx-auto px-5 pt-0.5 md:px-[1rem]">
		<div class="flex items-center w-full max-w-full">
			...
			<div class="self-start flex flex-none items-center text-gray-600 dark:text-gray-400">
				...
				<!-- .net 服务状态 -->
				<Tooltip content={$i18n.t('新服务端状态')}>
					<button
						id="new-chat-button"
						class=" flex {$NetSettings['net_service']
							? ''
							: 'md:hidden'} cursor-pointer px-2 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition"
						on:click={() => {
							initNewChat();
						}}
					>
