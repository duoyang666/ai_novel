<script lang="ts">
	...
	import { uploadDocToDB } from '$lib/apis/net';
	...
	const uploadDoc = async (file) => {
		let obj = {
			tags: JSON.stringify(tags.map((c) => c.name))
		};
		const res = await uploadDocToDB(file, obj).catch((error) => {
			toast.error(error);
			return null;
		});
		console.log(res);
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				...
				>
					<div class="mb-3 w-full">
						...
						>
							{#if inputFiles}
								{inputFiles.length > 0 ? `${inputFiles.length}` : ''} 个文档
							{:else}
					...
					<div class=" flex flex-col space-y-1.5">
						<div class="flex flex-col w-full">
							<div class=" mb-1.5 text-xs text-gray-500">{$i18n.t('分组')}</div>
