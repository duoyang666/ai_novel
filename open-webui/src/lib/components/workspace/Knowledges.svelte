<script lang="ts">
	...
	import { WEBUI_NAME, knowledges } from '$lib/stores';
	import { del_knowledges, get_knowledges } from '$lib/apis/net';
	import { createNewDoc, deleteDocByName, getDocs } from '$lib/apis/documents';
	import { SUPPORTED_FILE_TYPE, SUPPORTED_FILE_EXTENSIONS } from '$lib/constants';
	import { goto } from '$app/navigation';

	import Checkbox from '$lib/components/common/Checkbox.svelte';
	import EditDocModal from '$lib/components/knowledges/EditDocModal.svelte';
	import SettingsModal from '$lib/components/knowledges/SettingsModal.svelte';
	import AddDocModal from '$lib/components/knowledges/AddDocModal.svelte';
	...
	const deleteDoc = async (id) => {
		let res = await del_knowledges([id]);
		if (res) {
			toast.success(`删除成功`);
			loadList();
	...
	const deleteDocs = async (docs) => {
		let ids = docs.map((c) => c.id);
		let res = await del_knowledges(ids);
		if (res) {
			toast.success(`删除成功`);
			loadList();
		} else toast.error(`删除失败`);
	...
	let loadList = async () => {
		try {
			knowledges.set(await get_knowledges());
			knowledges.subscribe((docs) => {
				tags = docs.reduce((a, e, i, arr) => {
					//console.log(a, e, i, arr)
					return [...new Set([...a, ...(e?.tags.split(',') ?? []).map((tag) => tag)])];
				}, []);
			});
	...
	let filteredDocs;
	$: filteredDocs = $knowledges.filter(
		(doc) =>
			(selectedTag === '' || doc?.tags.split(',').includes(selectedTag)) &&
			(query === '' || doc.title.includes(query))
	);
</script>
...
{#key selectedDoc}
	<EditDocModal bind:show={showEditDocModal} bind:loadList={loadList} {selectedDoc} />
{/key}

<!-- 添加知识库 -->
<AddDocModal bind:show={showAddDocModal} bind:loadList={loadList} />
...
<div class="mb-3">
	<div class="flex justify-between items-center">
		<div class=" text-lg font-semibold self-center">{$i18n.t('Knowledge')}</div>
		...
		<div>
			...
			>
				...
				<div class=" text-xs">分割{$i18n.t('Settings')}</div>
			</button>
		</div>
	</div>
	<div class=" text-gray-500 text-xs mt-1">
		<!-- ⓘ {$i18n.t("在输入框中输入'$'号来选择并附带你的文档")} -->
	</div>
</div>
		...
		{:else}
			<div class="flex-1 flex w-full justify-between items-center">
				<div class="text-xs font-medium py-0.5 self-center mr-1">
					{filteredDocs.filter((doc) => doc?.selected === 'checked').length} 选中
				</div>
		...
		>
			...
			<div class=" flex flex-1 space-x-4 cursor-pointer w-full">
				<div class=" flex items-center space-x-3">
					<div class="p-2.5 bg-red-400 text-white rounded-lg">
						...
						{:else}
							...
							>
								...
								<circle class="spinner_qM83" cx="4" cy="12" r="2.5" /><circle
					...
					<div class=" self-center flex-1">
						<div class=" font-bold line-clamp-1">{doc.title}</div>
			...
			<div class="flex flex-row space-x-1 self-center">
				<button
					...
					on:click={async (e) => {
						e.stopPropagation();
						goto(`/workspace/knowledges/create?id=${encodeURIComponent(doc.id)}`);

						//location.href = '/workspace/knowledges/create';

						// var a = document.createElement('a');
						// a.href = '/workspace/knowledges/create';
						// //a.target = '_blank'; // 可选：在新标签页中打开
						// a.click();
					}}
				>
					<svg
