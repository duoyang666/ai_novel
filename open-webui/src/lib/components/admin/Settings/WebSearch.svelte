...
<script lang="ts">
	...
	const submitHandler = async () => {
		...
		let values = {
			url: WEBUI_OBJ.SEARCH2_URL,
			enabled: webConfig.search.enabled,
			count: webConfig.search.result_count
		};
		let res1 = await set_settings({
			ClassName: 'WebUi_Search',
			...
			<div>
				...
				{#if webConfig.search.engine !== ''}
					<div class="mt-1.5">
						...
						<!-- {:else if webConfig.search.engine === 'duoyang'}
							<div>
								<div class=" self-center text-xs font-medium mb-1">多样扩展 自定义地址</div>
								...
								<div class="flex w-full">
									<div class="flex-1">
										<input
											class="w-full rounded-lg py-2 px-4 text-sm dark:text-gray-300 dark:bg-gray-850 outline-none"
											type="text"
											bind:value={WEBUI_OBJ.SEARCH2_URL}
											autocomplete="off"
										/>
									</div>
								</div>
